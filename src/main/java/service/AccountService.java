...
public class AccountService {
	...
	public Account addAccount(String clientid, AddAccountDTO account) throws DatabaseException, BadDecimalException,
			BadParameterException, BadAccountTypeException, ClientNotFoundException {
		Account newAccount;
		try {
			double accountBalance = account.getAccountBalance();
			String balanceString = Double.toString(accountBalance);

			// make sure splitter[1] will not return null when balance doesn't have a
			// decimal
	...
	public List<Account> getAllAccount(String clientid)
		...
		try {
			...
			if (accounts.size() == 0) {
				throw new AccountNotFoundException(
						"Client don't have account with balance between " + min + " and " + max);
			}
			return accounts;
		} catch (SQLException e) {
			throw new DatabaseException(e.getMessage());
	...
	public Account getAccountByid(String clientid, String accountid)
			throws DatabaseException, BadParameterException, ClientNotFoundException, AccountNotFoundException {
	...
	public Account editAccount(String clientid, String accountid, Account account)
			throws DatabaseException, BadDecimalException, BadParameterException, ClientNotFoundException,
			AccountNotFoundException, BadAccountTypeException {
		Account newaccount;
		try {
			double accountBalance = account.getAccountBalance();
			String balanceString = Double.toString(accountBalance);
	...
	public void deleteAllAccount(String clientid)
			throws SQLException, ClientNotFoundException, DatabaseException, BadParameterException {
		try {
			int cid = Integer.parseInt(clientid);
			// check client id positive
			if (cid < 0) {
